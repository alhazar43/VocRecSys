...
class AdaptiveMIRTv2:
    ...
    def binary_prob(a, b, th):
        """Optimized binary probability"""
        return 1 / (1 + np.exp(-(np.dot(a, th) - b)))

    @staticmethod
    @jit(nopython=True, fastmath=True)
    def mc_binary_prob(a, th):
        """Optimized MC multi binary probability"""
    ...
    def mc_single_prob(a, th):
        """Optimized MC single probability"""
        expnt = np.dot(a.T, th)
    ...
    def compute_binary_infos(a_params, bin_b, th, indices):
        """Parallel computation of binary information"""
        n = len(indices)
        infos = np.zeros(n)
        for i in prange(n):
            idx = indices[i]
            prob = 1 / (1 + np.exp(-(np.dot(a_params[idx], th) - bin_b[idx])))
            infos[i] = prob * (1 - prob)
        return infos
    ...
    def next_item(self):
        ...
        # Split by item type for vectorized computation
        binary_mask = self.item_types[remaining] == 'binary'
        binary_indices = remaining[binary_mask]
        
        if len(binary_indices) > 0:
            # Use parallel computation for binary items
            binary_infos = self.compute_binary_infos(
                self.a_params, self.bin_b, self.est_th, binary_indices)
            infos[binary_indices] = binary_infos
        
        # Process other items
        other_indices = remaining[~binary_mask]
        if len(other_indices) > 0:
            infos[other_indices] = [self.log_like(self.est_th, item=i) for i in other_indices]
